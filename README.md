...
The same change for Raven itself — the launcher is generated by
`install_session_launcher()` in RavenLinux's `scripts/stages/stage-gui.sh` —
is `contrib/ravenlinux-session.d.patch` (`cd RavenLinux && patch -p1 <
…/contrib/ravenlinux-session.d.patch`). Once that ships in an image, the
runtime patch above becomes a no-op (it's marker-guarded).
